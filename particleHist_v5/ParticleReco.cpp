...
Event::pointpart loopPointer;
  ...
  for ( int i = 0; i < e.nParticles(); ++i ) {
    ...
    // pion mass for K0,
	EK0 += Utilities::Energy_calc( loopPx, loopPy, loopPz,
					           Constants::massPion );
	...
	if(loopPointer->q > 0){
		ELambda0 += Utilities::Energy_calc( loopPx, loopPy, loopPz,
  ...
  invK0      = Utilities::Invar_mass_calc( sumpx, sumpy, sumpz,
						               EK0 );
  invLambda0 = Utilities::Invar_mass_calc( sumpx, sumpy, sumpz,
						               ELambda0 );
